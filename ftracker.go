...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	avarageSpeed := meanSpeed(action, duration) // средняя скорость в км/ч
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	avarageSpeed := meanSpeed(action, duration) * kmhInMsec // средняя скорость в м/c
	heightInM := height / cmInM                             //рост в метрах
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(avarageSpeed, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	avarageSpeed := swimmingMeanSpeed(lengthPool, countPool, duration) // Средняя скорость
	return (avarageSpeed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
}
